...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        shoulder_left_px = (
            int(shoulder_left[0] * width),
            int(shoulder_left[1] * height),
        )
        shoulder_right_px = (
            int(shoulder_right[0] * width),
            int(shoulder_right[1] * height),
        )
        ...
        distance_to_camera = (
            shoulder_width_meters * focal_length
        ) / distance_px
        ...
        for landmark in mp_pose.PoseLandmark:
            ...
            if landmarks[landmark.value].visibility > 0.5:
                landmark_px = (
                    int(landmarks[landmark.value].x * width),
                    int(landmarks[landmark.value].y * height),
                )
        ...
        cv2.putText(
            frame,
            f"Distance: {distance_to_camera:.2f} meters",
            (50, 50),
